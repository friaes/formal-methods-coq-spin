...
(** ** FP - Higher-Order Functions - Functions that take functions as arguments *)
  ...
  *)
  ...
  even _and_ greater or equal than 8. *)

  Definition filter_even_gt7 (l : list nat) : list nat :=
    filter (fun n => andb (even n) (Nat.ltb 7 n)) l.
  ...
  Fixpoint map {X : Type} (f : X -> X) (l : list X) : list X :=
    match l with
    | [] => []
    | h :: t => (f h) :: (map f t)
    end.
  ...
  *)
  ...
  Definition animals : total_map string := (* string -> string *)
    fun s =>
      match s with
      | "cat" => "🐱"
      | "dog" => "🐶"
  ...
  Definition t_empty {A : Type} (v : A) : total_map A := (* string -> A *)
    (fun _ => v).

  Compute t_empty 0 "somekey". (* 0 is the default element *)
  Compute t_empty 0 "some other key". (* 0 is still the default element *)
...
(** ** TP - Proofs on Higher-Order Functions *)
 ...
 (** We can now prove some properties of our higher-order functions. *)
  ...
  Proof.
    ...
    induction l as [| h t IH].
    - simpl. reflexivity.
    - simpl. destruct (odd h) eqn: E.
      assert (H: forall (n : nat) (l : list nat), odd n = true -> countoddmembers' (n :: l) = S (countoddmembers' l)).
      + intros. unfold countoddmembers'. simpl. rewrite H. reflexivity.
      + apply H with h (filter odd t) in E.
        rewrite E. simpl. rewrite IH. reflexivity.
      + apply IH.
  (* FILL IN HERE *) Admitted.
  ...
  (x !-> v ; m) x = v.
  Proof.
    intros. unfold t_update. destruct (x =? x) eqn: E.
    - reflexivity.
  ...
  Proof.
    ...
    destruct (x =? y) eqn: E'.
    - apply eqb_eq in E'. unfold "<>" in E. apply E in E'. destruct E'.
    - reflexivity. 
  Qed.
  ...
  Proof.
    ...
    intros x'. destruct (String.eqb_spec x x').
